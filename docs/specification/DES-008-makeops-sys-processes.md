...
* **Applies Concepts:**
    * `MOD-007` (Pure Execution OS Boundaries: Translating arguments to `execvp` compatible C strings).
    * `MOD-005` (Asynchronous Execution and Multiplexing: Multiplexing stdout and stderr via `poll`).
    * `MOD-008` (System Signal Routing: Normalizing POSIX exit statuses).
    * `MOD-009` (Formal Verification & Static Memory Foundations: Safe conversion of POSIX integers to Ada variants).
...
* **Core Types & State:**
    * `Max_Arg_Length`: A static constant (1024) defining the maximum byte length of a single evaluated argument.
    * `Max_Args_Per_Command`: A static constant (32) defining the maximum number of arguments per execution.
