...
Before any graph operations can occur, the orchestrator must establish its reality. It parses the raw CLI arguments (`ALG-007`), resolves the hierarchical configuration cascade (`ALG-003`), establishes the physical working directory (`PLAT-007`), and parses the project's configuration file into memory. If any syntactical or semantic violations occur during parsing, the pipeline acts as the top-level exception handler, catching the domain errors and delegating them to the Contextual Error Rendering diagnostic engine (`MODEL-002`) before safely aborting execution.
...
* Returns a domain error (e.g., `Cycle_Detected`, `Missing_Variable`, `Path_Too_Long`, `Permission_Denied`) on pre-flight validation failures. Prints contextual errors directly to `stderr` via the Zero-Allocation Diagnostic Pattern (`MODEL-002`) during Phase 0.
...
1. *--- Phase 0: Initialization & Parsing ---*
    ...
    * Check `Parse_Result.Status`:
        * **If `Error`:**
            * Call `MODEL_002_Render_Contextual_Error(Global_Config.Project_Config, Parse_Result.Error_Type, Parse_Result.Line_Number, Parse_Result.Column_Number)`
            * Return `1`
        * **If `Success`:**
...
* **Constraints:** The pipeline MUST strictly adhere to the phase ordering. It MUST NOT perform on-the-fly variable evaluation during Phase 3. The `Virtual_Root` injection MUST be handled completely in memory and MUST NOT permanently mutate the global configuration state. All domain errors returned during the phases MUST be handled explicitly by the application's top-level execution logic (such as invoking Contextual Error Rendering via `MODEL-002` in Phase 0) to provide clean, contextual logs (`PLAT-012`) before exiting, completely avoiding the use of native Ada exception handlers for control flow.
...
* [8] [MODEL-002: Zero-Allocation Diagnostic Pattern](./MODEL-002-zero-allocation-diagnostics.md)
