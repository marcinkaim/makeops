...
# SPEC-012 MakeOps.Sys.Identity Package Specification
...
* **Out of Scope:** This package strictly queries the OS state. It MUST NOT contain the business logic for interpreting the `--allow-root` CLI flag, nor should it autonomously abort the application. Halting the execution pipeline is the responsibility of the Master Orchestration Pipeline (`ALG-009`).
...
* **Applies Concepts:**
    * `PLAT-010` (Security Context and Root Privileges: Preventing Workspace Pollution).
    * `PLAT-001` (Pure Execution and OS Bindings: Thin to Thick wrapper translation).
